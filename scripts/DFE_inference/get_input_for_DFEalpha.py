...
#Example usage:
#python3 get_input_for_DFEalpha.py -inPath '/home/vivak/rr_mu_demography_DFE/results/rr_fixed_mu_fixed/sim1_gene1_rep1.ms' \
...
for i in range(minRep, maxRep+1):
    ...
    s_not_anc_NS = d_ns2[2] #Number of polymorphic sites + number of fixed sites in the sample
    s_not_anc_S = d_s2[2]
    ns_0_sites = ns_0_sites + ((exonic_region_length - (exonic_region_length/3)) - d_ns2[2])
    s_0_sites = s_0_sites + ((exonic_region_length/3) - d_s2[2])
#Write to file in Grapes-friendly format    
with open(outFile, 'w') as f:
    f.write("1\n"+str(samples)+"\n")
    f.write(str(int(ns_0_sites)))
    f.write(' ')
